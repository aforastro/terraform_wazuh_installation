...
resource "local_file" "ssh_key1" {
  filename = "${aws_key_pair.aws_key_indexer.key_name}.pem"
  content = tls_private_key.indexer_key.private_key_pem
 
  
   provisioner "local-exec" {
    # openssl genrsa -out mykey.pem 1024
    # openssl rsa -in mykey.pem -pubout > mykey.pub
   command = "openssl rsa -in ./aws_key_indexer.pem -pubout >> ./aws_key_indexer.pem.pub"
 }
...
resource "local_file" "ssh_key2" {
  filename = "${aws_key_pair.aws_key_server.key_name}.pem"
  content = tls_private_key.server_key.private_key_pem
    ...
    # openssl genrsa -out mykey.pem 1024
    # openssl rsa -in mykey.pem -pubout > mykey.pub
   command = "openssl rsa -in ./aws_key_server.pem -pubout >> ./aws_key_server.pem.pub"
...
resource "local_file" "ssh_key3" {
  filename = "${aws_key_pair.aws_key_dashboard.key_name}.pem"
  content = tls_private_key.dashboard_key.private_key_pem
    ...
    # openssl genrsa -out mykey.pem 1024
    # openssl rsa -in mykey.pem -pubout > mykey.pub
   command = "openssl rsa -in ./aws_key_dashboard.pem -pubout >> ./aws_key_dashboard.pem.pub"
 }
file_permission = "0400"
